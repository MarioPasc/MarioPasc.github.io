...
2. **Start the server in the background**:
   ```bash
   cd /home/mpascual/misc/MarioPasc.github.io && bundle exec jekyll serve --livereload > /tmp/jekyll-serve.log 2>&1 &
   echo $! > /tmp/jekyll-serve.pid
   ```

3. **Wait for readiness** (Jekyll takes a few seconds to compile):
   ```bash
   for i in $(seq 1 15); do
     curl -s -o /dev/null -w "%{http_code}" http://localhost:4000/ | grep -q "200" && break
     sleep 1
   done
   ```
   If the server does not respond after 15 seconds, check `/tmp/jekyll-serve.log` for errors and report them.

4. **Confirm with Playwright** — Use the Playwright MCP to navigate to `http://localhost:4000/` and take a snapshot. This proves the server is live and the site renders correctly.
...
| `webrick` missing | It is in the Gemfile; run `bundle install` |
